...
- **Applying without reviewing** → you inherit autogenerate's mistakes (see
  "Review the migration before applying" below).
...
### Back up before a destructive-risk migration

A migration that touches existing rows is a **destructive-risk operation**: it
changes the schema — and sometimes the data — of a database that already holds
real values. The classic case is adding a NOT NULL column to a table with data
(you must backfill before the constraint can apply; a mistake loses or
corrupts rows). The rule: **before applying any migration that modifies
existing data, take a backup. Always.**
...
```bash
# backup
docker compose exec db pg_dump -U app_user -d pynance_db > /tmp/backup_before_migration.sql

# restore (if needed)
docker compose exec db psql -U app_user -d pynance_db < /tmp/backup_before_migration.sql
```

Seconds of cost versus irreplaceable data: this is why the backup is done
before every schema change on data you care about, in dev as much as in prod.
See `07-auth.md` for a concrete example (the `user_id` backfill migration) —
it applied exactly this practice.
