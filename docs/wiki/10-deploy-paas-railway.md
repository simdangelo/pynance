# Modulo 10 — Deploy con la strategia A: Railway in pratica
...
"tutto in uno") usando **Railway**, e costruiamo il deploy della nostra app
...
backend serve sia le API sia i file del frontend compilato. Su Railway
...
entrambe le strade. Cambia solo *dove* la metti: qui su Railway, senza
server da gestire.
...
Railway impostiamo solo la `DATABASE_URL`. Nessun comportamento esistente
cambia.
...
Railway può buildare l'app in due modi: con la sua build automatica
...
migrazioni — e che Railway esegue come un solo servizio.
...
- **La porta è `${PORT:-8000}`.** Railway (come ogni PaaS) non usa una porta
...
- **`--proxy-headers`** resta: anche qui c'è un proxy davanti (quello di
  Railway), e questa flag fa sì che uvicorn legga gli header standard del
...
Il CD (deploy automatico) **non** è un workflow GitHub: è Railway stessa. La
...
## Il deploy su Railway, passo passo

Una volta che il codice è pronto e la CI è verde, il deploy su Railway si fa
dalla sua interfaccia web, in pochi passi:

1. **Creare un account** su Railway e collegare l'account GitHub.
2. **Nuovo project → Deploy from GitHub repo** e scegliere il repo Pynance
   e la branch `deploy`.
3. **Aggiungere un Postgres**: dal pannello "New" → Database → PostgreSQL.
   Railway crea il database e mostra la sua `DATABASE_URL`.
4. **Configurare il web service**: Railway rileva il `Dockerfile` alla radice
   del progetto e lo usa per buildare l'immagine.
5. **Impostare le variabili d'ambiente** del servizio (sotto la sezione
   Variables):
   - `DATABASE_URL` — la stringa data da Railway nel passo 3;
   - `SECURE_COOKIES=true` — il cookie di sessione deve essere `Secure`
     perché l'app gira su HTTPS (ADR 0005);
   - `ALLOWED_HOSTS=["...up.railway.app"]` — TrustedHostMiddleware deve
...
6. **Deploy**: Railway builda l'immagine, avvia il container (l'entrypoint
   esegue le migrazioni), e assegna un dominio `*.up.railway.app` con HTTPS
   automatico.
7. **Verifica**: aprire `https://...up.railway.app/api/health` — deve
...
Se leggi il deploy di Railway e ti chiedi "dove sono il Caddy, il firewall, il
reverse proxy?" — la risposta è: **non ci sono, e va bene così**. La
strategia A esiste proprio per questo: Railway fa da reverse proxy, gestisce
...
nel repository, non nel pannello di Railway.
...
- [ ] `https://...up.railway.app/api/health` → `{"status":"ok"}`
...
- [ ] un push sulla branch fa ripartire il deploy (CD via Railway)